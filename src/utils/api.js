...
const API_URL = `http://localhost:3008/api/phones?`
...
export const api = () => {
  ...
  return {
    ...
    getPhones: async (query) => {
      const endPoint = `http://localhost:3008/api/phones`
    ...
    findAdByID: async (id) => {
        const endPoint = `http://localhost:3008/api/phones/detail/${id}`;
        const response = await axios.get(endPoint);
